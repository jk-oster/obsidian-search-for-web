...
//                 if (key == 'liveSearch' || key == 'showInPageIcon' || key == 'port') document.querySelector('#' + key).checked = value;
...
//         browser.storage.sync.get({ port: false }).then((data) => {
//             if (data.port) portElem.classList.remove('hidden');
