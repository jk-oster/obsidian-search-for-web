...
export default defineComponent({
  ...
  computed: {
    showPopup(): boolean {
      ...
      return Boolean(this.notes?.length > 0 && store.searchString?.length > store.minChars && store.show);
    },
    computedNotes(): any[] {
  ...
  async mounted() {
    ...
    this.mode = store.searchUrls.split(',').some(url => location.origin.includes(url)) ? 'search' : 'urlMatch';
    this.initSearch();
